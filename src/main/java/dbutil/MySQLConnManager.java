...
public class MySQLConnManager {
    ...
    public static Connection creatConnectionByIPandDB(String ip, String dbName) {
        String url = String.format("jdbc:mysql://%s:3306/%s?useSSL=false&rewriteBatchedStatements=true&useUnicode=true&characterEncoding=UTF-8", ip, dbName);
        Connection dbConnection = null;
        try {
            // 把JDBC驱动类装载入Java虚拟机中
            Class.forName(DRIVER);
